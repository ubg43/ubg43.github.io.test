...
const updateTrendRailState=()=>{const sub=document.querySelector('#trendingSection .section-sub');if(!sub)return;sub.textContent=globalTrending.ready&&Object.keys(globalTrending.counts).length?'Most played across UBG43':globalTrending.ready?'Collecting global play activity across UBG43…':globalTrending.loading?'Loading global play activity across UBG43…':'Global trend data will update automatically'};
