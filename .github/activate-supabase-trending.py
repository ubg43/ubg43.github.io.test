...
runtime_path = Path('.github/stable-runtime-hotfix.py')
...
elif 'SUPABASE_TRENDING_URL' not in s:
    raise SystemExit('Could not find the existing global trending constants')

increment = re.compile(r"async function incrementGlobal\(c\)\{.*?\n\}\nconst recordPlay=", re.S)
...
const recordPlay="""
if increment.search(s):
    s = increment.sub(new_increment, s, count=1)
...
if loader.search(s):
    ...
    raise SystemExit('Could not find loadGlobalTrending in stable runtime')
